...
ld computeNormalizationFactor(FILE *fp) {
	...
	while (!feof (fp)){
		ld temp;
		fscanf (fp, "%Lf", &temp); 
		maxAmplitude = std::max(maxAmplitude,abs(temp)-DCshift);
	}
	ld factor = 5000.000000/(maxAmplitude*1.0);  
	return factor;
}
void detectWord(ld energyArr[],long totaLframes,int *start,int *end,int live) {//start and end are out parameters
	ld baseEnergy = 0.0;
	for(int i=1;i<=std::min((ld)5,(ld)totaLframes);i++)
		baseEnergy+=energyArr[i];
	baseEnergy/=5;// to detect start and end of the word, we are trying to calculate base energy taking first 5 frames(500 samples)
	int factor = 10;
	for(int i=1;i<=totaLframes-2;i++)
	...
	if(live==1) {
		for(int i=*start;i<=std::min((ld)(*start+5),(ld)totaLframes);i++)
			baseEnergy+=energyArr[i];
		baseEnergy/=5;// to detect start and end of the word, we are trying to calculate base energy taking first 5 frames(500 samples)
		factor = 5;
		for(int i=*start;i<=totaLframes-2;i++)
			if(energyArr[i]>=factor*baseEnergy && energyArr[i+1]>=factor*baseEnergy && energyArr[i+2]>=factor*baseEnergy) {//if data is above 3x then base energy that means start of the frame
				*start = i;
				break;
		}
	...
	baseEnergy = 0;
	for(int i=totaLframes;i>=std::max((ld)1,(ld)totaLframes-4);i--)
		baseEnergy+=energyArr[i];
	baseEnergy/=5;
	factor = 5;
	for(int i=totaLframes;i>=5;i--) {
		if(energyArr[i]>=factor*baseEnergy && energyArr[i-1]>=factor*baseEnergy && energyArr[i-2]>=factor*baseEnergy && energyArr[i-3]>=factor*baseEnergy && energyArr[i-4]>=factor*baseEnergy) {//if data is below 3x then base energy that means start of the frame
			*end = i;
			break;
		}
	}
...
void extractStableFrame(char word[],int utterance,int live) {//live =1 for live data
	...
	detectWord(energyArray,frames,&start,&end,live);//live = 0 for pre recorded files
	//printf("\nStart : %d",start);
	//printf("\nEnd : %d",end);
